function [smse,msll,t_pred,t_up] = run_gPoE_LoG_GP(X,Y)
% IN: 
...
% MoE-LoG-GP learning
disp('Pre-training of gPoE-LoG-GP ...');
gPoE_LoG_GP = GPOE_LOG_GP();
gPoE_LoG_GP.sigL = ls;
gPoE_LoG_GP.sigF = sf;
gPoE_LoG_GP.sigN = sn;

gPoE_LoG_GP.pts = 100;
gPoE_LoG_GP.loadHyp = false;
gPoE_LoG_GP.wo = 100;
gPoE_LoG_GP.divMethod = 2;
gPoE_LoG_GP.xSize = size(X,2);
try
    gPoE_LoG_GP.N = 18;
    gPoE_LoG_GP.setupData();
catch
    gPoE_LoG_GP.N = 16;
    gPoE_LoG_GP.setupData();
end
...
for i=1:Npretrain
    gPoE_LoG_GP.update(X(i,:)',Y(i,:));    
end

disp('Online Learning with gPoE-LoG-GP ...');
...
for i = Npretrain+1:size(X,1)
    ...
    yte = Y(i,:);
    tic;
    [mu(i-Npretrain,1),sig(i-Npretrain,1),lik(i-Npretrain,1)] = gPoE_LoG_GP.predict(xte,yte); 
